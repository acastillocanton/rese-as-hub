...
⚠️ **Pendiente del usuario (operativo):** publicar a Producción + reconectar (deroga la nota de §4.26 Bloque F que dejaba "Testing está bien"). Mientras siga en Testing, el token vuelve a caducar cada ~7 días. **Follow-up menor (fuera de alcance):** la pill derivada del **dashboard** (§4.21) puede enmascarar el `error` mostrando "Places API verde" si hay `place_id` (Places está apagado → rama obsoleta); /fichas sí usa `isBpError`.
