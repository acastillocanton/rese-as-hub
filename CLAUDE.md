...
| feat · Comercial multi-oficina ("escrituradora", mig 031): `sales` con `cross_location` sin ficha fija; elige la ficha (Oropesa/Castellón/Valencia) por cliente (`clients.location_id`); landing+matcher usan la ficha del cliente. ⚠️ mig 031 PENDIENTE de aplicar (§4.60) | ✅ código (2026-06-30) |
...
⚠️ **Limitaciones conocidas (aceptadas):** (a) **no reclama huérfanas multi-ficha** en Verificación (`current_user_location()` devuelve null para ella → no ve `unmatched` de las fichas que sirve; sus reseñas se auto-atribuyen por nombre/tiempo/mención, y admin/gestor reasigna si hace falta — RLS multi-ficha sería fase 2). (b) **No aparece en el parte Excel semanal** por departamento (department null → la salta `groupActiveSalesByDepartment`; decisión de negocio). (c) Sus reseñas **no las ve el director** de la oficina (RLS por equipo), pero sí admin/gestor y sí cuentan en la valoración pública de la ficha. (d) En el ranking su `branch` sale "—".
