...
- `MESSAGE_TEMPLATES: MessageTemplateDef[]` — 3 plantillas base, ids estables `post_visita` (la histórica = default) · `reavivar` · `breve`. Cada una `build(brand)` con placeholders `{nombre_cliente}/{nombre_comercial}/{url}`. `post_visita.build === getDefaultReviewMessageTemplate` (no se duplica el texto).
- `resolveTemplate(id, brand, overrides)` — devuelve la versión personalizada del comercial si existe y no está en blanco, o la base. Los overrides se guardan **con placeholders** (sin renderizar) y con la marca ya escrita.
- `SavedTemplates = Partial<Record<MessageTemplateId, string>> | null` — shape de `profiles.message_templates`.
...
**Persistencia (mig 019)**: columna `profiles.message_templates jsonb` (nullable). Sin RLS nueva — la escritura es server-only vía service-client filtrando por `id = auth.uid()` (mismo patrón que el avatar). Añadida también a los tipos hand-maintained de [lib/supabase/types.ts](lib/supabase/types.ts) (`profiles.Row`/`Insert`).

**UI**:
- [ShareBlock.tsx](app/(sales)/clientes/ShareBlock.tsx): selector de 3 pestañas (pills) encima del textarea. Al elegir una, el textarea se rellena con `renderMessage(resolveTemplate(...))` (override del comercial o base) ya con nombre de cliente + URL. Los retoques manuales del textarea son **efímeros** (se pierden al cambiar de pestaña — aceptado). Link discreto "Editar mis plantillas →".
...
- Sección **"Mis plantillas"** [app/(sales)/panel/plantillas/](app/(sales)/panel/plantillas/): `page.tsx` (server, carga override + brand) + `MyTemplatesEditor.tsx` (client, edita las 3 con chips de comodines + "Restablecer a la original" por plantilla) + `actions.ts::saveMessageTemplates` (Zod: cada plantilla opcional, max 1000, **si no vacía debe contener `{url}`**; blanco → revierte a base; todo vacío → NULL; audit `update_message_templates`). Accesible vía link en `ShareBlock` y card en `/panel/enlace` (no cabe tab nueva en `MobileTabBar`).

**Tests** ([lib/__tests__/messaging.test.ts](lib/__tests__/messaging.test.ts)): 3 ids en orden, `post_visita` base == default histórico, cada base con los 3 placeholders + marca correcta, y `resolveTemplate` (override no vacío / blanco / id no coincidente).
