...
- **office_director** (director de oficina, migración 011) — **admin scoped a UNA ficha** (`profiles.location_id`). Misma IA que admin pero las pantallas filtran por su location y los endpoints validan que cualquier `location_id` de input coincida con el suyo. Hace todo lo que admin (conectar/desconectar OAuth, editar Place ID, gestionar sus comerciales) pero solo sobre su ficha. NO accede a `/gestores` ni `/ajustes` (son globales). Solo el admin general lo invita/edita/elimina desde `/gestores`.
...
- **Proyecto**: `zejwmznusszqlwhevaqv.supabase.co`. Migraciones **001-010 aplicadas**; **011 + 012 pendientes de aplicar en Supabase Dashboard EN ESE ORDEN, en dos ejecuciones separadas**. 007 = índices compuestos en `reviews`. 008 = `actor_id` + policy `audit_log_self_insert`. 009 = enum `review_source_enum` (`business_profile`/`places_api`/`manual`) + columna `source` en `reviews`. 010 = columna `removed_at` + índice parcial + view `reviews_active`. **011 = añade valor `'office_director'` al enum `role_enum`** (aislado por la limitación 55P04 de Postgres: un nuevo valor de enum no puede usarse como literal en la misma transacción en que se añadió). **012 = resto del rol `office_director`**: constraint `role_requires_location`, helper `current_office_location()` y policies RLS para director sobre `locations`, `profiles` `role='sales'`, `reviews`, `clients`, `share_links`.
