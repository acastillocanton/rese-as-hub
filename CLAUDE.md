...
| fix(matching) · §4.47 (clic-temporal a único comercial) pasa de `counted` a **`pending`** — no paga sola, la confirma un humano; falsos positivos cross-mercado (p.ej. reseña rusa→comercial rumano). Las 14 ya atribuidas degradadas a pending — §4.47 | ✅ (2026-06-10) |
...
>
> **Re-check programado el 2026-06-23** (routine remota `trig_01LnriLHvqFNWmjTv2xnj3MP` — solo recordatorio: el agente remoto NO puede correr el script por falta de secretos). **Señal de que llegó**: abrir `/fichas/[id]/conectar` y ver el SELECTOR de fichas en vez del 429 (o `node scripts/check-bp-quota.mjs` en local, script gitignored). Si pasados >10 días hábiles sigue a 429, responder al hilo del caso para dar un toque.
>
...
> ⚠️ **CAMBIO 2026-06-10 — ahora entra como `pending`, NO `counted`.** El clic-temporal es la señal más débil (ni nombre ni mención) y un clic genérico en ráfaga producía falsos positivos cross-mercado que **se pagaban solos** (caso real: reseña rusa "Елена Тесля"/"Супер" atribuida a **Adriana Mihalascu**, comercial del mercado **rumano**, solo porque alguien clicó su enlace 6 min antes; además era un clon Places+BP que contaba doble). Como Business Profile **no aporta** ninguna señal que ligue el clic con la reseña (Google es dueño del dato, §4.38), la única corrección sensata es **no auto-contar**: la §4.47 PROPONE el comercial en `pending` y un humano confirma en Verificación. Las 14 reseñas ya atribuidas por esta vía se degradaron a `pending` (audit `demote_temporal_to_pending`). El lever real para subir la calidad es la **mención** en el texto (§4.38, vía copy del mensaje), no el clic.
...
**Hueco cerrado** (en [lib/matching/attribute-review.ts](lib/matching/attribute-review.ts), `attributeBySingleCommercialInWindow`): tras fallar la atribución por nombre (`primaryAttribution`) y por mención (`attributeByCommercialMention`), si seguimos en `unmatched`, se mira la ventana corta: si en ella hubo clics de **EXACTAMENTE UN** comercial → se **propone** ese comercial en **`pending`** (confianza 70), **sin cliente** (desde 2026-06-10; antes era `counted` automático — ver banner). No cuenta ni paga hasta que un humano confirma.
...
- **`client_id` siempre null**: si `primaryAttribution` no llegó a `PENDING_THRESHOLD`, ningún candidato tenía buen parecido de nombre → adivinar cliente sería arriesgado (anti-fraude mig 015 podría degradar la principal). Lo afina el humano al confirmar.
...
- **`reason: "single_commercial_temporal_pending"`** en `match_evidence` (antes `counted_by_single_commercial_temporal`). La reseña aparece en Verificación → "Pendientes" con el comercial propuesto; confirmar/rechazar/reasignar la resuelve (§4.46).
...
- **Pendientes deliberados**: (a) **deep-link a reseña concreta** — NO se hace: Google no da URL pública por `reviewId` de la API (§4.25); se mantiene "Ver reseña" → lista. (b) **soft-delete automático** (`reconcileRemoved`) — diferido: causó falsos positivos con Places (§4.20); reactivar solo desde el cron BP y con capa `last_seen_at` (threshold de N runs) si se decide. (c) **reconciliar las ~156 reseñas históricas de Places** para publicarles respuesta por API — NO se hace (decisión "solo nuevas", §4.48): requeriría migración + barrido contra Google casando por autor/estrella/fecha. Se quedan en flujo manual.
