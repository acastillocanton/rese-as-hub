...
- **office_director** (director de oficina, migraciones 011 + 012 + 013) — **admin scoped a SU EQUIPO** (`profiles.director_id`). Cada director gestiona un subset de comerciales dentro de una ficha (p.ej. en Internacional, un director por idioma). Una location puede tener varios directores, cada uno con su `director_id`. Hace todo lo que admin sobre sus comerciales (verificar reseñas del equipo, ver dashboard scoped al equipo, exportar Excel del equipo) y sobre SU ficha (conectar/desconectar OAuth, editar Place ID — `/fichas` sigue scoped por `location_id`). NO accede a `/gestores`, `/ajustes` ni `/directores` (son admin-only). Solo el admin general invita/edita/elimina directores desde `/directores`.
...
- **Proyecto**: `zejwmznusszqlwhevaqv.supabase.co`. Migraciones **001-012 aplicadas**; **013 pendiente de aplicar en Supabase Dashboard**. 007 = índices compuestos en `reviews`. 008 = `actor_id` + policy `audit_log_self_insert`. 009 = enum `review_source_enum` (`business_profile`/`places_api`/`manual`) + columna `source` en `reviews`. 010 = columna `removed_at` + índice parcial + view `reviews_active`. **011 = añade valor `'office_director'` al enum `role_enum`** (aislado por la limitación 55P04 de Postgres: un nuevo valor de enum no puede usarse como literal en la misma transacción en que se añadió). **012 = resto del rol `office_director`**: constraint `role_requires_location`, helper `current_office_location()` y policies RLS para director sobre `locations`, `profiles` `role='sales'`, `reviews`, `clients`, `share_links`. **013 = el office_director pasa de scope por location a scope por equipo** (`profiles.director_id`): nueva columna auto-referencial + reescritura de policies de profiles/reviews/clients/share_links a `director_id = auth.uid()`. La policy de `locations` se mantiene por `location_id` (acceso a su ficha sigue por oficina). Permite varios directores en la misma ficha, cada uno con su equipo (p.ej. uno por idioma en Internacional).
