...
| feat(sync) · Soft-delete AUTOMÁTICO de reseñas borradas en Google (mig 028): sello `missing_since` + umbral 24h, solo cron BP, restauración manual — §4.20 | ⚠️ DESACTIVADO 2026-06-17 (falsos positivos sobre counted; `AUTO_REMOVE_ENABLED=false`; vuelve a solo-manual) |
...
| fix(oauth) · Token de Google caduca a los 7 días en modo "Testing" (tumbó las 7 fichas). Código: `markSyncError` marca `oauth_status='error'` ante fallos de auth (`isOAuthAuthError`) → pill "Error OAuth" + botón "Reconectar" + mensaje claro en "Sincronizar". Consent screen publicado a **Producción** + 7 fichas reconectadas (tokens ya no caducan) (§4.58) | ✅ (2026-06-17) |
...
| fix(verificacion) · Paginar la bandeja (25/pág, `Pagination.tsx`): la pestaña "Atribuidas" (337 reseñas) intentaba pintarlas todas de golpe y la navegación no completaba → "no abría". Sin migración (§4.62) | ✅ (2026-07-20) |
| feat(verificacion) · Filtros de búsqueda en la bandeja (nombre del autor · ficha · comercial · valoración · rango de fechas opcional), form GET que actúa dentro de la pestaña; la paginación respeta filtros (conteo aparte). Sin migración (§4.62) | ✅ (2026-07-20) |
| fix(nav) · Soft-nav de Next entre pestañas/páginas del MISMO segmento NO commitaba en **build de producción** con listas pesadas (RSC 200 pero React no aplicaba → "no abría"); dev NO lo reproduce. Solución: hard-nav `<a>` (chips + `Pagination` prop `hardNav` + enlaces de estado); el form de filtros ya es GET. `/resenas/respuestas` sigue en soft-nav. Verificado en prod build (§4.62) | ✅ (2026-07-20) |
| fix(matching) · Corroboración de `pending` por clic en enlace del MISMO cliente en ventana corta → `counted` (§4.47). El rescate con-cliente solo corría sobre `unmatched`; un parecido de nombre PARCIAL (apodo que comparte el nombre de pila → 55+8=63 pending) dejaba pending una reseña con evidencia fuerte (cliente abrió su enlace específico y reseñó a los minutos). Caso "luis Luisito"/Marisol. Solo reseñas nuevas del cron. Sin migración (§4.47) | ✅ (2026-07-22) |
| fix(sync) · 504 intermitentes del cron BP horario (`FUNCTION_INVOCATION_TIMEOUT`, 5/9 runs el 07-22): `fetchWithRetry` respetaba `Retry-After` de Google sin tope dentro de lambdas con `maxDuration=60` (tope Hobby). Cap de 8s por espera ([lib/google/business-profile.ts](lib/google/business-profile.ts)) + presupuesto de 45s en el loop de fichas (`skipped_time_budget`, antes del lock; 200 parcial en vez de 504) + reintento único tras 90s en `sync-reviews-hourly.yml`. Verificado E2E en prod (200 en 17s; run #328 OK). Sin migración | ✅ (2026-07-23) |
...
⚠️ **Detección automática DESACTIVADA de nuevo (2026-06-17) — falsos positivos.** Tras reactivarse el 2026-06-11 (mig 028), el reconcile marcó **5 reseñas `counted` legítimas de comerciales** (5★, no duplicadas — Katalin Tolvay, Елена Тесля, Viktorija Dulko, Judit Silaghine Juhász, Victoria Batiste) como eliminadas el 06-14..06-16. **Causa raíz**: el "invariante de ventana" asume que la API v4 de Business Profile devuelve un feed estable/completo, pero **Google omite reseñas intermitentemente entre llamadas** (eventual-consistency / paginación flaky) — exactamente la lección de Places (§4.20 histórico). 5 de 47 BP counted, en días distintos → ausencias aleatorias, no patrón. El umbral de 24h no basta porque la ausencia puede sostenerse varios runs por mala suerte. Las 5 se restauraron a mano y el soft-delete vuelve a ser **SOLO MANUAL** (estado pre-mig-028) vía flag `AUTO_REMOVE_ENABLED=false` en [lib/google/sync-business-profile.ts](lib/google/sync-business-profile.ts). El reconcile (`reconcileRemovedBp` + helper puro `decideReconcileRemoved`) y la columna `missing_since` se conservan **inertes** por si en el futuro se diseña una detección robusta (exigir fetch paginado completo + ausencia sostenida de N días). ⚠️ **No volver a poner `AUTO_REMOVE_ENABLED=true`** sin resolver antes la no-fiabilidad del feed de Google.

<details><summary>Histórico: cómo funcionaba el reconcile (mig 028, 2026-06-11 → 2026-06-17)</summary>

`reconcileRemovedBp` en [lib/google/sync-business-profile.ts](lib/google/sync-business-profile.ts), decisión en el helper puro [lib/cron/reconcile-removed.ts](lib/cron/reconcile-removed.ts) (`decideReconcileRemoved`, tests propios). **Invariante de ventana**: la API v4 lista por `updateTime` desc → el fetch contiene TODAS las reseñas con `updateTime >= min(updateTime bajado)`; toda fila BP de BD con `google_created_at > ese mínimo` debería aparecer — si no aparece, desapareció de Google. **Capa anti-falsos-positivos**: la primera ausencia solo sella `reviews.missing_since` (mig 028); el `removed_at` llega únicamente tras **24h de ausencia sostenida** (`AUTO_REMOVE_THRESHOLD_HOURS`); si reaparece antes, se limpia el sello. Acotado a `source='business_profile'`. `audit_log action='review_auto_removed'`; contador `summary.auto_removed`. (El invariante de ventana resultó no sostenerse contra la flakiness del feed — ver arriba.)
</details>

La restauración sigue siendo **manual** (`restoreReview`, que limpia también `missing_since`); ningún flujo automático limpia un `removed_at`. (La vieja `reconcileRemoved` de `sync-places.ts` sigue sin llamarse — Places está apagado.)
...
> - **PENDIENTE (no hecho a propósito)**: deep-link a reseña concreta (Bloque C punto 7) — Google no expone URL pública por reviewId, se queda "Ver reseña" a la lista. ~~Soft-delete automático (Bloque C punto 6)~~ → reactivado el 2026-06-11 (mig 028) y **DESACTIVADO de nuevo el 2026-06-17 por falsos positivos** (vuelve a solo-manual, ver §4.20).
>
...
**Corroboración de `pending` por clic en enlace de cliente (ampliado 2026-07-22, `corroboratePendingByClientLink`):** el rescate con-cliente de arriba solo corría cuando el match por nombre quedaba en **`unmatched`** (nombre 0). Pero un parecido **parcial** (autor firma con apodo que solo comparte el nombre de pila → `nameSimilarity=55` +8 temporal = 63 → **`pending`**) dejaba fuera una señal MÁS fuerte que la de §4.47: el cliente abrió SU enlace específico y reseñó a los minutos. Paradoja: si el nombre casaba 0 contaba (80), pero si casaba a medias se quedaba pending. **Fix:** si `primary` quedó en `pending` apuntando a un comercial+cliente concretos Y existe un clic en el enlace **específico de ESE MISMO cliente** dentro de la ventana corta (≤30 min), sube a `counted` (confianza 80, `reason: "counted_by_pending_client_link_corroboration"`, preserva comercial+cliente). Conservador: exige que el clic sea del MISMO cliente al que apunta el pending y en la ventana corta. Caso real que lo motivó: **Marisol / "luis Luisito"** (cliente "luis marquez hermoso" abrió su enlace y reseñó a los 87 s, firmando con apodo). Reversible en Verificación → "Atribuidas".

Sin migración, sin cambios en el cron ni en BD: `loadCandidates` ([lib/cron/process-reviews.ts](lib/cron/process-reviews.ts)) ya carga todos los `share_links` de la ficha en ventana (incl. `client_id` y `client_full_name`), y la función usa `sales_id` + `opened_at` + `client_id`. **Solo aplica a reseñas nuevas** que entren por el cron — las ya en BD (como la de Eduuu, o la de "luis Luisito") se reclaman/confirman a mano. Tests en [lib/matching/__tests__/attribute-review.test.ts](lib/matching/__tests__/attribute-review.test.ts) (incl. el escenario real de Eduuu, el clic en enlace específico que atribuye cliente, multi-cliente sin desambiguar, el límite de los 30 min, y la corroboración de pending del caso "luis Luisito" + sus negativos).
...
- **Pendientes deliberados**: (a) **deep-link a reseña concreta** — NO se hace: Google no da URL pública por `reviewId` de la API (§4.25); se mantiene "Ver reseña" → lista. (b) ~~soft-delete automático~~ reactivado el 2026-06-11 (mig 028) y **DESACTIVADO de nuevo el 2026-06-17 por falsos positivos sobre counted** → solo-manual (ver §4.20). (c) **reconciliar las ~156 reseñas históricas de Places** para publicarles respuesta por API — NO se hace (decisión "solo nuevas", §4.48): requeriría migración + barrido contra Google casando por autor/estrella/fecha. Se quedan en flujo manual.
...
### 4.58 Caducidad del token OAuth de Google + estado de error visible (2026-06-17)

**Incidente:** el refresh token OAuth murió en las **7 fichas** a la vez (`invalid_grant: token expired or revoked`), 7 días exactos tras conectarlas. Causa: la app estaba en modo **"Testing"** en Google Cloud y Google **caduca los refresh tokens externos a los 7 días** en ese modo. Síntoma doble: (a) dejaron de entrar reseñas en todas las fichas; (b) la UI seguía mostrándolas "conectadas" sin botón de reconectar.

**Arreglo de fondo (operativo, NO código):** publicar el consent screen de **"Testing" → "En producción"** en el proyecto `628454280082` (estado de publicación = lo que quita la caducidad de 7 días; la verificación de Google es aparte y no bloquea el funcionamiento). Requiere consent screen completo (homepage, `/privacidad`, `/terminos`, dominio autorizado `marinadorconstrucciones.com` verificado en Search Console) — un publish con campos a medias da **500** en `accounts.google.com/signin/oauth/consent`. La cuenta **no es Workspace**, así que el camino "Internal" (sin caducidad ni verificación) NO está disponible. Tras publicar, reconectar las 7 fichas (incógnito, una sola cuenta, marcar la casilla de Business Profile).

**Arreglo de código (este commit) — hacer el fallo VISIBLE:**
- ⚠️ Bug de fondo: `markSyncError` ([lib/google/sync-business-profile.ts](lib/google/sync-business-profile.ts)) solo escribía `oauth_last_sync_error`, **nunca cambiaba `oauth_status`** → la ficha seguía en `connected`, y en /fichas el botón "Reconectar" solo aparece si `oauth_status !== 'connected'` (sale "Desconectar" en su lugar) → admin ciego.
- Helper puro `isOAuthAuthError(message)` ([lib/google/business-profile.ts](lib/google/business-profile.ts), tests en [lib/google/__tests__/oauth-auth-error.test.ts](lib/google/__tests__/oauth-auth-error.test.ts)): `true` solo ante firmas de auth inequívocas (`invalid_grant`, `no_refresh_token`, `token refresh/exchange failed`, `invalid_client`, scope insuficiente, 401/unauthenticated). **Conservador**: 5xx/429/red → `false` (no saca del cron a una ficha sana por un blip).
- `markSyncError`: si `isOAuthAuthError(error)` → además marca `oauth_status='error'`. Efecto: pill "Error OAuth" + botón "Reconectar" directo en /fichas, y la ficha sale del filtro `oauth_status='connected'` del sync (no se machaca un token muerto cada hora). La vuelta a `connected` la hace `linkGoogleLocation` ([app/(admin)/fichas/actions.ts](app/(admin)/fichas/actions.ts), ya ponía `oauth_status='connected'` + `oauth_last_sync_error=null`) al reelegir la ficha tras reconectar.
- `SyncNowButton` ([components/ui/SyncNowButton.tsx](components/ui/SyncNowButton.tsx)): en la rama "0 nuevas", si hubo errores, muestra "No se pudo sincronizar: N fichas con error de conexión a Google. Reconéctalas en Fichas." (antes decía engañosamente "Sin reseñas nuevas").

### 4.59 Auditoría de seguridad (2026-06-17)

Auditoría enfocada en 4 superficies (auth/sesiones, RLS/IDOR, inyección/datos no confiables, secretos/cabeceras/crons) vía 4 subagentes en paralelo. **Veredicto: postura sólida** — RLS blindada con test de regresión, secretos aislados (service-key y tokens OAuth nunca al cliente), cabeceras completas, escapado XSS en emails, CSRF en OAuth, re-gating de rol en código en endpoints sensibles. Un único fallo grave, ya corregido.

**🔴 CRÍTICO (corregido):** `inviteReviewsManager` ([app/(admin)/gestores/actions.ts](app/(admin)/gestores/actions.ts)) era la ÚNICA action de invitación sin `assertAdmin()` → iba directa a `createInvitedProfile(role:'reviews_manager')` con service-client. Como el middleware solo valida el *pathname* (no qué server action se despacha), un `sales` podía invocarla desde una ruta permitida y autoinvitarse como gestor (rol privilegiado). Añadido `assertAdmin()`. Verificado que `inviteSales`/`inviteOfficeDirector` sí gateaban. **Regla reforzada (§4.36): toda server action que escriba con service-client DEBE re-gatear rol/propiedad en el propio action, nunca confiar solo en el middleware.**

**Arreglos aplicados (código):**
- **SSRF** en `expandShareUrl` ([app/(profile)/resenas/maps-url-actions.ts](app/(profile)/resenas/maps-url-actions.ts)): `redirect:"follow"` seguía la cadena de redirects a cualquier host → ahora redirect MANUAL con allowlist de host de Google por salto + timeout 5s + tope de saltos (SSRF ciego, estaba gated a admin/gestor).
- **Excel**: `excelSafe()` también en la columna `google_review_id` ([app/api/export/reviews/route.ts](app/api/export/reviews/route.ts)) + `maxDuration=60`.
- **`ALLOWED_TYPES`** ([app/auth/confirm/route.ts](app/auth/confirm/route.ts)) recortado a los 3 tipos OTP que la app emite (`email`/`magiclink`/`invite`); fuera `recovery`/`signup`/`email_change`.
- **CSP** ([next.config.ts](next.config.ts)): `'unsafe-eval'` se quita en **producción** (solo se mantiene en dev para Turbopack). Si tras un deploy algo no hidrata, revertir.
- **Brevo** ([lib/email/brevo.ts](lib/email/brevo.ts)): el warning de SMTP incompleto ya no loguea los emails de destinatarios (PII), solo el conteo.
- **`public/robots.txt`** añadido (`Disallow: /`).

**Migración 030** ([supabase/migrations/030_security_audit_hardening.sql](supabase/migrations/030_security_audit_hardening.sql)) — ✅ **aplicada en Supabase** (verificado el 2026-06-30: policy `audit_log_self_insert` ausente, trigger `trg_audit_commission` y función `audit_commission_change` presentes): (1) quita `audit_log_self_insert` (mig 008) que dejaba a cualquier autenticado fabricar entradas de auditoría a su nombre — `recordAudit` usa solo service-client, así que no rompe nada; (2) trigger `trg_audit_commission` que registra cambios de `commission_rate`/`commission_cap` a nivel de BD (un gestor/director podía cambiarlos por PostgREST directo sin pasar por la action que audita). Se eligió el trigger en vez de congelar columnas en WITH CHECK porque el camino legítimo (updateSales con cookie-client) SÍ debe poder cambiarlas.

**No es vulnerabilidad (revisado):** `office_director` puede llamar a `/api/export/reviews` pero el endpoint fuerza su `location_id` y la RLS acota las hojas resumen a su equipo — es por-diseño (lo usa `/manager/export`); rechazar el rol rompería su export.

**PII en `public/help/` (RESUELTO 2026-06-17):** las capturas del centro de ayuda (con comisión € y nombres reales de un comercial) ya **NO se sirven sin auth**. `middleware.ts` añade `/help/:path*` al matcher (el patrón general excluía `.png`) y `pathAllowedForRole` permite `/help/*` solo a usuarios autenticados de cualquier rol (igual que `/ayuda`); un anónimo es redirigido a `/login`. No se movieron ficheros ni se tocó `HelpFigure`. Test de regresión en [lib/__tests__/route-access.test.ts](lib/__tests__/route-access.test.ts).

**Pendientes de decisión del usuario:**
- **Dependencias** (`npm audit`): 1-2 altas (`tmp` path traversal, transitiva de build) + moderadas (`uuid` vía `exceljs`, `postcss`/`next`). Riesgo runtime bajo; `npm audit fix` resuelve `tmp` sin breaking.
- **Bajos latentes** (hoy seguros): `.or()` de PostgREST por interpolación (slug validado), `claimReview` apoya `client_id` en código (`clientBelongsToSales`), encriptar `oauth_refresh_token` en reposo (open question #8 de spec.md).

✅ **Hecho el 2026-06-17:** consent screen del proyecto `resenas-inseryal` **publicado a "En producción"** (tipo "Usuarios externos"; "Internal" NO disponible por no ser Workspace) con la marca completa (homepage + `/privacidad` + `/terminos` + dominio autorizado `marinadorconstrucciones.com`, verificado en Search Console). **Sin logo** (subir logo forzaría verificación). Las **7 fichas reconectadas** con la cuenta que administra los perfiles (**`socialmedia.inseryal@gmail.com`**, es gmail pero es la dueña de las fichas) → refresh tokens nuevos **ya NO caducan a los 7 días** (la caducidad era exclusiva del modo Testing). Esto **deroga** la nota de §4.26 Bloque F ("Testing está bien"). ⚠️ La app sigue **sin verificar** (Producción unverified): funciona con la pantalla de aviso "app no verificada" (Avanzado → Continuar) y tope de 100 usuarios; la **verificación de Google** (para quitar el aviso) queda como follow-up no urgente. **Follow-up menor (fuera de alcance):** la pill derivada del **dashboard** (§4.21) puede enmascarar el `error` mostrando "Places API verde" si hay `place_id` (Places apagado → rama obsoleta); /fichas sí usa `isBpError`.

### 4.60 Comercial multi-oficina · "escrituradora" (mig 031)

La persona que acompaña al cliente en la firma de la escrituración pide reseñas como un comercial más, pero **no pertenece a una sola ficha de Google**: escritura viviendas de cualquier promoción, así que sus clientes caen en **Oropesa** (playa), **Castellón** o **Valencia**. La reseña debe aterrizar en la ficha de Google de la oficina correcta (valoración pública de esa oficina) pero **atribuirse a ella** (su panel, ranking, comisión).

**Modelado (decisión de negocio):** NO es un rol nuevo. Es un `sales` con flag `profiles.cross_location = true` y **sin `location_id`**. Reutiliza TODO el rol `sales` (la RLS de sales gatea por `sales_id`, no por ficha → funciona con location nula). Cada uno de sus **clientes** guarda su ficha destino en `clients.location_id` (la elegida al crear el cliente). La landing y el `share_link` usan la ficha del cliente; el matcher ya filtra candidatos por `share_links.location_id` → la reseña de esa ficha se le atribuye por nombre/tiempo sin tocar el algoritmo.

**Migración 031** ([supabase/migrations/031_cross_location_producer.sql](supabase/migrations/031_cross_location_producer.sql)) — ✅ **aplicada en Supabase** (verificado 2026-06-30; las 3 fichas Oropesa/Castellón/Valencia quedaron marcadas `escrituracion_target`): `profiles.cross_location boolean default false` + relaja `role_requires_location` (mig 012) para permitir productor sin ficha cuando `cross_location` + `clients.location_id` (nullable, FK locations; NULL = hereda la del sales, comportamiento normal) + `locations.escrituracion_target boolean default false` (define el set del selector; backfill Oropesa/Castellón/Valencia por `name ilike`) + reescribe `profiles_self_update` congelando `cross_location` (regla §4.36; en `FROZEN_COLUMNS` del test).

**Cambios de código:**
- **Landing** ([lib/landing.ts](lib/landing.ts)): `effectiveLocationId = client.location_id ?? sales.location_id`; el `place_id` y `share_links.location_id` salen de ahí (antes siempre `sales.location_id`).
- **Alta de cliente**: `createClientRecord` ([app/(sales)/clientes/actions.ts](app/(sales)/clientes/actions.ts)) acepta `locationId`; si el perfil es `cross_location` lo **exige** y valida que sea `escrituracion_target`. UI: `<select>` de fichas en [NewClientButton](app/(sales)/clientes/NewClientButton.tsx) (props `crossLocation` + `fichaOptions`), threadeado desde `/clientes` y `/panel`.
- **Gestión** ([app/(admin)/comerciales/actions.ts](app/(admin)/comerciales/actions.ts) + `InviteSalesButton`/`SalesEditCard`): casilla "Comercial multi-oficina (escrituración)". Solo admin/reviews_manager (un `office_director` no, está acotado a su oficina). Al marcarla: `location_id`/`director_id`/`department` quedan null.
- **Matcher**: helper [lib/cron/cross-location-roster.ts](lib/cron/cross-location-roster.ts) añade a los `cross_location` al roster (`commercialsByLocation`) de cada ficha donde tienen un cliente → habilita el rescate por mención (§4.38) en esas fichas. Llamado por ambos crons.
- **Marca por cliente**: el mensaje ("...soy X de {marca}") usa la marca de la ficha elegida del cliente, no la del perfil (que sería `DEFAULT_BRAND` al no tener ficha). Resuelto en `/clientes`, `/clientes/[slug]` y el diálogo del `NewClientButton`.
- **Enlace genérico oculto**: `/panel/enlace` y la tarjeta "Tu enlace personal" del `/panel` no se muestran para `cross_location` (su `/c/{slug}` sin cliente no resuelve ficha → todo es por-cliente). `/panel/enlace` muestra una tarjeta que dirige a "Mis clientes".
- **Reclamar huérfanas multi-ficha (mig 032 — ✅ aplicada, verificado 2026-06-30)**: en Verificación puede reclamar (`claimReview`, botón "Es mía") huérfanas de **cualquier ficha de escrituración** (no solo de "su" ficha, que no tiene). La mig 032 amplía las policies `reviews_unmatched_location_select` (ver) y `reviews_sales_claim_update` (reclamar) con `is_cross_location_producer() AND location_id in (escrituracion_target)`; el WITH CHECK del claim no se relaja (sigue exigiendo `sales_id = auth.uid()` + `counted`). La pantalla lista por RLS → las huérfanas aparecen solas. En código, `assertReviewInScope` (sales) y el branch de cliente-nuevo de `claimReview` (que hereda la ficha de la reseña, mig 031) contemplan `cross_location`.
...
### 4.62 Bandeja de Verificación: paginación + filtros + hard-nav (2026-07-20)

Dos features + un fix de navegación en `/resenas/verificacion` ([app/(profile)/resenas/verificacion/page.tsx](app/(profile)/resenas/verificacion/page.tsx)).

**Paginación (25/pág).** La pestaña "Atribuidas" (`?state=counted`, 337 reseñas) antes hacía `.limit(500)` y renderizaba cientos de tarjetas cliente pesadas de golpe → la pestaña "no abría". Ahora paginación server-side con `.range()` + [components/ui/Pagination.tsx](components/ui/Pagination.tsx) (reutilizado de `/resenas/respuestas`, §4.48). El total por pestaña sale de una query de conteo aparte (`count:"exact", head:true`, mismos filtros). Cambiar de pestaña resetea a página 1 (los chips no arrastran `page`).

**Filtros de búsqueda** (solo admin/gestor/responsable; el `sales` no los ve). Formulario GET que actúa DENTRO de la pestaña activa: **buscar por nombre del autor** (`ilike author_name`), **ficha** (`location_id`), **comercial** (`sales_id`, solo en counted/pending — en unmatched/removed el sales es null), **valoración** (`rating` 1-5) y **rango de fechas OPCIONAL** (`from`/`to` sobre `google_created_at`; vacío = todas, la bandeja es histórica y no imponemos periodo por defecto). Los mismos filtros se aplican a la query de lista Y a la de conteo (mantener en sync). `buildHref` preserva estado+filtros+página en la paginación; los chips y "Limpiar" resetean filtros. Empty-state específico "sin resultados con filtros". Sin migración.

⚠️ **fix(nav) — la lección importante ([[nav-soft-nav-prod-heavy-lists]]):** la **soft-nav de Next (`<Link>`) entre URLs del MISMO segmento** (solo cambia searchParam) **NO hace commit en el build de producción** cuando la lista es pesada (RSC 200 completo, sin errores en consola, pero React no aplica → URL no cambia, "no abre"). No hay `loading.tsx` en cambios de searchParam del mismo segmento, así que tampoco hay feedback. Manifestó dos veces aquí (al paginar y al filtrar). `/resenas/respuestas` (lista más ligera) NO lo sufre con el mismo `Pagination`.
- **Solución robusta:** los chips de pestaña, los enlaces de estado y `Pagination` (prop nueva `hardNav`) usan **carga completa de página `<a href>`** en vez de `<Link>`. El formulario de filtros ya es hard-nav (GET). `/resenas/respuestas` deja `Pagination` en soft-nav (`hardNav` default false, sin cambios).
- ⚠️ **`npm run dev` NO reproduce el bug** (dev no prefetchea/optimiza igual → la soft-nav sí commita). **Verificar SIEMPRE contra un build de producción** (`npm run build && npx next start`) en Chrome real (chrome-devtools MCP; el navegador de preview integrado tampoco renderiza bien esta app de Next 15).
- Descartado como causa: `count:"exact"` en la query de lista y el prefetch (`prefetch={false}` NO lo arregló). La causa raíz exacta (React concurrent no committeando el árbol pesado en prod) quedó sin diagnosticar; hard-nav es suficiente y robusto.
